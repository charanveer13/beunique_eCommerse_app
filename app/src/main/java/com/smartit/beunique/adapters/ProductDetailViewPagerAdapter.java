...
public class ProductDetailViewPagerAdapter extends PagerAdapter {
    ...
    public ProductDetailViewPagerAdapter(Context context, EOAllProductData eoAllProductsData) {
    ...
    public Object instantiateItem(@NonNull ViewGroup view, int position) {
        ...
        if(!ObjectUtil.isEmpty ( productsData.getAssociations ( ) )) {
            if(!ObjectUtil.isEmpty ( productsData.getAssociations ( ).getImages ( ) )) {
                Picasso.get ( ).load ( BASE_URL + "api/images/products/" + productsData.getId ( ) + "/" + this.productsData.getAssociations ( ).getImages ( ).get ( position ).getId ( ) + "/?ws_key=RIQADSXL8ARE7C76LWQLEKLTIDFH4RPV" )
